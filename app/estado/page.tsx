...
export default function EstadoPage() {
  ...
  const bairroReplays = events.filter(e => e.event === 'arcade_replay_click' && e.slug === 'bairro-resiste');
  ...
  const bairroReplayRate = bairroRunCount > 0 ? Math.round((bairroReplays.length / bairroRunCount) * 100) : 0;
    ...
    <div className={styles.container}>
      <Section>
        ...
        <Card className={styles.fullCard}>
          ...
          <div className={styles.signalGrid}>
            <div className={styles.signalItem}>
              <p className={styles.signalLabel}>Runs (Base real)</p>
              <p className={styles.signalValue}>{bairroRunCount}</p>
              <p className={styles.signalNote}>Meta: 30 runs</p>
            </div>
            <div className={styles.signalItem}>
              <p className={styles.signalLabel}>Fase média atingida</p>
              <p className={styles.signalValue}>{bairroAvgPhase}</p>
              <p className={styles.signalNote}>1: Sondagem, 2: Aceleração, 3: Caos</p>
            </div>
            <div className={styles.signalItem}>
              <p className={styles.signalLabel}>Setor mais crítico (Moda)</p>
              <p className={styles.signalValue}>{String(bairroWorstHotspotMode).toUpperCase()}</p>
            </div>
            <div className={styles.signalItem}>
              <p className={styles.signalLabel}>Conversão View → Start</p>
              <p className={styles.signalValue}>
                {(() => {
                  const views = metrics.gamesSorted.find(g => g.slug === 'bairro-resiste')?.initiated || 0;
                  return views > 0 ? Math.round((bairroRunCount / views) * 100) : 0;
                })()}%
              </p>
            </div>
          </div>
          ...
          <div className={styles.tableWrap}>
             <h4>Funil de Validação</h4>
             <table className={styles.table}>
                <thead>
                   <tr>
                      <th>Etapas</th>
                      <th>Volume</th>
                      <th>Conversão Etapa</th>
                   </tr>
                </thead>
                <tbody>
                   {(() => {
                      const gameRow = metrics.gamesSorted.find(g => g.slug === 'bairro-resiste');
                      const views = gameRow?.initiated || 0; 
                      const starts = bairroRunCount;
                      // Conclusão no Bairro Resiste é timeout (sobreviver 90s)
                      const completions = bairroResisteRuns.filter(e => e.metadata?.reason === 'timeout').length;
                      const replays = events.filter(e => e.event === 'arcade_replay_click' && e.slug === 'bairro-resiste').length;

                      return [
                         { label: '1. Visualizações (Landing)', value: views, conv: '-' },
                         { label: '2. Inícios (Start Click)', value: starts, conv: views > 0 ? `${Math.round((starts/views)*100)}%` : '0%' },
                         { label: '3. Conclusões (90s sobrevivência)', value: completions, conv: starts > 0 ? `${Math.round((completions/starts)*100)}%` : '0%' },
                         { label: '4. Replays (Interesse recorrente)', value: replays, conv: `${bairroReplayRate}%` }
                      ].map((row, i) => (
                         <tr key={i}>
                            <td className={styles.gameTitle}>{row.label}</td>
                            <td className={styles.numeric}>{row.value}</td>
                            <td className={styles.numeric}>{row.conv}</td>
                         </tr>
                      ));
                   })()}
                </tbody>
          ...
          <p className={styles.techNote}>
            Status: LIVE_BUT_EARLY. O objetivo é atingir 30 runs para validar se o tuning de dificuldade e o &quot;premium visual&quot; sustentam a retenção esperada.
          </p>
        </Card>
<Card className={styles.fullCard}>
